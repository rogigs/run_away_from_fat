...
class WeightLifting(HUD):
    ...
    def _weight(self):
        myfont = pygame.font.Font("assets/font/FreePixel.ttf", 50)
    ...
    def _control_height(self):
        if self._height_machine <= 275:
            self._end_game = True
            self._player_win = False
            self._height_machine = self._height_begin
            self._height_player = self._height_begin
        if self._height_player <= 275:
            self._end_game = True
    ...
    def weightlifting(self, strength,sound):
        ...
        return self._result_game() if not self.quit else None
